...
export const getActor = (actorId) => {
    return fetch(`https://imdb-api.com/en/API/Name/k_xujmt7ax/${actorId}`)
    .then((response) => response.json())
}
...
export const getUserGames = (userId) => {
    return fetch(`http://localhost:8088/games?_userId=${userId}`)
    .then((response) => response.json())  
} 

export const getProject = (projectId) => {
    return fetch(`https://imdb-api.com/en/API/Title/k_xujmt7ax/${projectId}`)
    .then((response) => response.json())
}